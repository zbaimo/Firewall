"""
增强型防火墙执行器
基于iptables的高级防火墙管理功能
支持：规则持久化、批量操作、规则验证、性能优化
"""
import subprocess
import ipaddress
import json
...
class IptablesManager:
    ...
    def __init__(self, db, config: Dict):
        ...
        logger.info(f"✓ iptables管理器初始化完成")
    ...
    def ban_ip(self, ip: str, reason: str = "Manual ban", 
        ...
        try:
            ...
            # 添加iptables规则
            comment = self._format_comment(reason, duration_seconds)
            cmd = [
            ...
            if result.returncode == 0:
                logger.info(f"✓ 封禁IP成功: {ip} (原因: {reason})")
                
                # 保存到数据库
                self._save_ban_record(ip, reason, duration_seconds)
                
                return True
            else:
                logger.error(f"封禁IP失败: {ip}, 错误: {result.stderr.decode()}")
                return False
    ...
    def _save_ban_record(self, ip: str, reason: str, duration: Optional[int]):
        """保存封禁记录到数据库"""
    ...
    def check_expired_bans(self):
        ...
        try:
            ...
            for ban in expired_bans:
                logger.info(f"自动解封过期IP: {ban.ip}")
                self.unban_ip(ban.ip)
                
            if expired_bans:
                logger.info(f"✓ 已解封 {len(expired_bans)} 个过期IP")
